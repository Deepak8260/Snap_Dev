...
| --- | --- | --- |
| [`provider.tf`](provider.tf) | AWS Provider Configuration | Configures AWS provider bound to region variable `var.aws_region`. |
| [`versions.tf`](versions.tf) | Terraform Core Version Rules | Requires `terraform >= 1.0.0` and `hashicorp/aws ~> 5.0`. |
| [`data.tf`](data.tf) | AMI Lookup Data Source | Dynamically retrieves latest Ubuntu 22.04 LTS Jammy HVM SSD AMI ID from Canonical (`099720109477`). |
| [`tls.tf`](tls.tf) | RSA Key Generator | Generates 4096-bit RSA SSH private key (`tls_private_key.jenkins_agent`). |
| [`keypair.tf`](keypair.tf) | AWS Key Pair | Registers generated public key into AWS EC2 Key Pairs (`aws_key_pair.terraform_key`). |
| [`security-groups.tf`](security-groups.tf) | AWS Security Groups | Defines `master_sg` (Ports 22, 8080) and `agent_sg` (Ports 22, 5000, 8081, 9090, 9100, 3000, 30080). |
| [`ec2.tf`](ec2.tf) | AWS EC2 Instances | Provisions `aws_instance.master` and `aws_instance.agent`, injecting rendered UserData templates. |
| [`variables.tf`](variables.tf) | Variable Definitions | Declares input variable schemas, defaults, and descriptions. |
| [`outputs.tf`](outputs.tf) | Output References | Outputs Master/Agent Public IPs, SSH connection strings, and Jenkins Web UI URL. |
| [`userdata/master.sh.tpl`](userdata/master.sh.tpl) | Master Bootstrap Template | Shell template installing Docker, Java 17, Jenkins LTS, writing JCasC & Groovy files, and starting Jenkins. |
| [`userdata/agent.sh.tpl`](userdata/agent.sh.tpl) | Agent Bootstrap Template | Shell template installing Docker, Java 17, and injecting controller public key into `authorized_keys`. |
